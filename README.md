...
2) Создать файл с переменными окружения — рекомендуется `env/dev.env` (или корневой `.env`). Приложение автоматически подхватит `env/dev.env`, а затем (при наличии) `.env`.
